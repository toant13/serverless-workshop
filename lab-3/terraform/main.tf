...
resource "aws_lambda_function" "serverless-user-profile-lambda" {
  filename = "../lambda/user-profile/Lambda-Deployment.zip"
  source_code_hash = "${base64sha256(file("../lambda/user-profile/Lambda-Deployment.zip"))}"
...
resource "aws_lambda_function" "serverless-custom-authorizer-lambda" {
  filename = "../lambda/custom-authoriser/Lambda-Deployment.zip"
  source_code_hash = "${base64sha256(file("../lambda/custom-authoriser/Lambda-Deployment.zip"))}"
